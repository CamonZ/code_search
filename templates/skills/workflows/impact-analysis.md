...
code_search --format toon code calls-to MyApp.Payments charge

# For a specific arity
code_search --format toon code calls-to MyApp.Payments charge 2
```
...
code_search --format toon code reverse-trace MyApp.Payments charge --depth 5

# Increase depth for widely-used functions
code_search --format toon code reverse-trace MyApp.Repo insert --depth 10
```
...
code_search --format toon code depended-by MyApp.PaymentsWeb
```
...
code_search --format toon code accepts Payment.t

# Who returns this type?
code_search --format toon code returns Payment.t
```
...
code_search --format toon code boundaries MyApp.Payments

# Check cluster membership
code_search --format toon code clusters MyApp.Payments
```
...
code_search --format toon code hotspots MyApp.Payments
...
code_search --format toon code calls-to MyApp.Accounts get_user
# Result: 15 direct callers

# 2. Trace backwards
code_search --format toon code reverse-trace MyApp.Accounts get_user --depth 4
# Result: Shows call chains from controllers, jobs, other contexts

# 3. Check module impact
code_search --format toon code depended-by MyApp.Accounts
# Result: 8 modules depend on Accounts

# 4. See if it's a hotspot
code_search --format toon code hotspots MyApp.Accounts
...
code_search --format toon code depended-by MyApp.LegacyPayments
...
code_search --format toon code calls-to MyApp.LegacyPayments
# Shows exactly which functions are still called

# 3. Check for type usage
code_search --format toon code struct-usage LegacyPayment.t
