...
use db::types::{Call, ModuleGroupResult, ModuleGroup};
...
fn build_dependent_caller_result(target_module: String, calls: Vec<Call>) -> ModuleGroupResult<DependentCaller> {
...
impl Execute for DependedByCmd {
    ...
    fn execute(self, db: &dyn db::backend::Database) -> Result<Self::Output, Box<dyn Error>> {
        ...
        Ok(build_dependent_caller_result(self.module, calls))
    }
}
