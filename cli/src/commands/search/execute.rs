...
impl Execute for SearchCmd {
    ...
    fn execute(self, db: &dyn db::backend::Database) -> Result<Self::Output, Box<dyn Error>> {
        match self.kind {
            SearchKind::Modules => {
                let modules = search_modules(db, &self.pattern, self.common.limit, self.common.regex)?;
                Ok(SearchResult {
            ...
            SearchKind::Functions => {
                let functions = search_functions(db, &self.pattern, self.common.limit, self.common.regex)?;
