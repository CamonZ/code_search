...
ORDER BY loc.end_line - loc.start_line + 1 DESC, loc.module, loc.name
LIMIT {}"#,
            self.limit
        ))
    }
...
mod tests {
    ...
    fn test_large_functions_query_age() {
        ...
        assert!(compiled.contains("(loc.end_line - loc.start_line + 1) >= $min_lines"));
        // AGE requires ordering by expression, not alias
        assert!(compiled.contains("ORDER BY loc.end_line - loc.start_line + 1 DESC"));
    }
